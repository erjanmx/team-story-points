...
class Counter extends Component {
  ...
  handleSubscribe(msg) {
    if (msg.name === 'add_vote') {
      ...
      this.setState((prevState) => ({
        members: prevState.members.map(member => ({ id: member.id, name: member.name, vote: null }))
      }));
    }
  }
  ...
  render() {
    ...
    if (this.state.members.length > 0 && this.state.members.length === this.getVotedMembers().length) {
      const min = _.minBy(this.state.members, 'vote').vote;
      const max = _.maxBy(this.state.members, 'vote').vote;
      // const avg = _.avgBy(this.state.members, 'vote').vote;
      result = max;
    }
